...
여러 컴퓨터를 오가며 작업하므로, 세션 맥락을 `PROGRESS.md`에 기록해
다음 세션이 어디서든 이어받을 수 있게 한다.

- **세션 시작 시**: 먼저 `PROGRESS.md`를 읽고 마지막 상태/다음 할 일을 파악한다.
- **세션 종료 시(또는 의미 있는 단계 완료 시)**: `PROGRESS.md`를 갱신한다.
    - 오늘 한 작업(완료된 것)
    - 현재 상태 / 마지막으로 건드린 파일
    - 다음에 할 일
    - 미해결 이슈나 결정 대기 중인 사항
...
`PROGRESS.md`는 "지금 어디까지 왔나"의 단일 기록점이다. 최신 상태를
