...
- **단계**: 구현 순서 1~7번 + KHOA 전자해도 배경 완료 + **STR 데이터셋 대응(1차) 완료** +
  ...
  **품질·이벤트 목록 구간 묶음 표시 완료** + **타선 UI 다듬기(이름 없으면 id 툴팁, 사이드바
  목록) 완료** + **에러 배너 닫기 버튼 + CSV 로딩 표시 완료**.
  전부 `dev` 브랜치에 커밋·푸시 완료. `main`은 `dev`보다 17커밋 뒤(PR #1 이후로 `main` 미갱신).
  분석가가 준 `str 데이터 분석.xlsx`(실 데이터 아님, 컬럼 사전) 기반. 브라우저 확인 끝.
  **실제 STR CSV는 아직 안 옴** — 다음 진전은 그게 와야 가능.
- **마지막으로 건드린 파일**: `src/App.tsx` / `src/components/FileLoader.tsx` /
  `src/components/TrafficLoader.tsx`
...
### 2026-08-07 (계속 3) — 에러 배너 닫기 버튼 + CSV 로딩 표시

UI 다듬기 후보 4개 중 미뤄뒀던 나머지 둘(에러 배너 닫기, 로딩 중 표시)을 이어서 진행.

- `src/App.tsx`: `error`/`trafficError` 배너를 `<p>` 한 줄에서 `flex` 행으로 바꿔 ✕ 닫기
  버튼 추가. 새 파일을 성공적으로 다시 로드하기 전까진 계속 떠 있던 문제 해결
- `src/App.tsx`/`FileLoader.tsx`/`TrafficLoader.tsx`: `trackLoading`/`trafficLoading`
  상태 추가. `handleFile`/`handleTrafficFile`을 `try/finally`로 감싸 로드 중
  "불러오는 중…" 텍스트 표시 + 관련 버튼(파일 선택/드래그앤드롭/예시 항적/타선 데이터)
  비활성화로 중복 요청 방지
  - **한계**: `await file.text()` I/O 구간은 이 표시로 커버되지만, 그 뒤 papaparse의
    동기 파싱 자체는 여전히 메인 스레드를 막는다. 완전한 스트리밍 전환(청크 단위 파싱)은
    실제로 수백 MB급 파일이 왔을 때 재검토하기로 함(이미 PROGRESS.md에 적혀 있던 이슈) —
    지금은 파일 I/O 구간만이라도 피드백을 주는 선에서 마무리
- 검증: `tsc -b`/`eslint src`/`npm run build` 통과. 브라우저에서 (1) 필수 컬럼 없는 CSV로
  에러 배너 띄운 뒤 ✕로 닫히는 것 확인, (2) 정상 파일 로드 후 버튼이 다시 활성화되고
  "불러오는 중" 문구가 안 남는 것(회귀 없음) 확인. 실제 로딩 스피너가 뜨는 순간은
  파일이 작아 화면 캡처로 못 잡음(로직·최종 상태로 검증)
