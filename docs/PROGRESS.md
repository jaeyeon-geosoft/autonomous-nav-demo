...
- **단계**: 구현 순서 1~4번 완료, 브라우저에서 동작 확인까지 끝. 5·6번도 대부분 함께 들어감
...
SPEC.md 구현 순서 7번 — 데이터 품질 검증 + 이슈 목록/하이라이트. 검사 항목 4가지:

1. **위치 점프**: 인접 포인트 간 거리/시간으로 속도를 역산해 비현실적으로 큰 구간
2. **결측치**: 주요 필드가 비어있는 포인트 (timestamp/lat/lon 결측은 파싱 단계에서
   이미 제외되고 `droppedRows`로만 집계됨 → 여기서는 sog/cog/hdg 담당)
3. **timestamp 이상**: 역전, 지나치게 큰 간격
4. **범위 벗어남**: 위경도 범위 밖, 속도 음수, heading 0~360 밖

작업 순서 제안:
- `src/data/quality.ts`에 순수 함수로 `findIssues(points): Issue[]` 작성 → Node로 검증
- 이슈 목록 패널 추가(클릭하면 해당 시점으로 `seek`)
- 지도에 이상 구간 마젠타 하이라이트
- 스크러버를 **속력 프로파일 리본**으로 교체하고 이상 지점을 마젠타 눈금으로 표시
  (5번에서 남겨둔 것. 이게 이 도구의 시그니처 — 재생 전에 수상한 구간이 먼저 보이게)

색: 마젠타(`--color-alert: #ff3d9a`)는 이상 구간 전용으로 예약해둠. 다른 용도로 쓰지 말 것.

### 알아둘 것

- **검증 습관**: 순수 로직은 임시 스크립트를 만들어 `npx vite-node ./_check.ts`로
  Node에서 돌려보고 지웠음. 이 방식으로 3번에서 38개 케이스를 잡았음.
  단, **React 렌더링 버그는 이걸로 못 잡음** — 3번에서 무한 렌더 루프가 그대로 통과했고
  브라우저에서야 발견됨. UI 변경은 반드시 브라우저에서 눈으로 볼 것
- 브라우저 확장(Claude in Chrome)이 연결되면 에이전트가 직접 화면을 볼 수 있어
  이런 왕복이 줄어듦. 이번 세션에는 미연결 상태였음
- **미해결/대기**:
